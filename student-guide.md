...
>
> ` "A radio station broadcasts multiple programs, which are assigned to hosts through a daily broadcasting schedule." `

### B. My tables (aim for 4+)

| Table name | What it stores | Columns (incl. primary key) |
|---|---|---|
| Users | Registered accounts and their access levels | `id` (PK), `username`, `password`, `role` |
| Stations | Radio stations with unique frequencies | `id` (PK), `name`, `frequency`, `created_at` |
| Hosts | Presenters/DJs who host the shows | `id` (PK), `name`, `bio`, `contact_email` |
| Programs | Radio shows/contents owned by stations | `id` (PK), `title`, `genre`, `station_id` (FK) |
| Schedules | Daily airtime slots linking programs to hosts | `id` (PK), `program_id` (FK), `host_id` (FK), `air_date`, `start_time` |
...
|---|---|---|---|
| 1 | Every **program** belongs to a **station**. | `programs.station_id` | `stations.id` |
| 2 | Every **schedule** belongs to a **program**. | `schedules.program_id` | `programs.id` |
| 3 | Every **schedule** belongs to a **host**. | `schedules.host_id` | `hosts.id` |

### D. My roles & permissions (2+ roles)

| Feature | Role 1: Admin | Role 2: Viewer (Fan/Public) |
|---|---|---|
| View radio stations & daily schedules | ✅ Yes | ✅ Yes |
| Add/Edit/Delete radio stations & programs | ✅ Yes | ❌ No |
| Manage hosts & assign broadcasting schedules | ✅ Yes | ❌ No |
| Search and filter programs by genre or host | ✅ Yes | ✅ Yes |

### E. My pages & access

| Page | What it does | Is it a form? | Who can access |
|---|---|---|---|
| index.php | Displays all available radio stations and their frequencies. | ❌ No | All (Admin & Viewer) |
| station_detail.php | Shows the details of a specific station along with its programs and today's schedule. | ❌ No | All (Admin & Viewer) |
| manage_programs.php | Allows the addition of new programs or editing/deleting existing ones. | 📜 Yes (Add/Edit Form) | Admin only |
| manage_schedule.php | Used to create and assign daily airtime slots to hosts for different programs. | 📜 Yes (Schedule Form) | Admin only |
